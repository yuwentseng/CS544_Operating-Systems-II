...
mon_backtrace(int argc, char **argv, struct Trapframe *tf)
{
    ...
    ebp = read_ebp();
    // note to self: pay attention to int vs. pointer arithmetic here
    ...
    while(ebp){
        ...
        cprintf("         %s:%d: %.*s+%d\n", dbinfo.eip_file, dbinfo.eip_line, dbinfo.eip_fn_namelen, dbinfo.eip_fn_name, eip - dbinfo.eip_fn_addr);
        ebp = *((uint32_t *)ebp);
    }
    return 0;
}
